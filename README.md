...
### 10. 「今、何を学ぶべきか」を提示するAIトレンド分析エージェント

単なる求人検索に留まらず、特定のスキル（React, Python等）が2026年現在、市場でどう評価されているかをGoogle検索でリアルタイム調査する機能を実装。初心者が次に踏み出すべき一歩を、具体的かつ親しみやすい言葉でアドバイスします。
...
### 1. 3つの言語・技術の連携（Next.js × Laravel × Python）

* **課題**
画面（Next.js）、サーバー（Laravel）、AI解析（Python）の3つの層に分かれているため、バグが起きたときに「どの場所でエラーが起きているか」を特定するのが難しかったです。
* **解決策**
ブラウザの通信記録（Networkタブ）、Laravelのログファイル、Pythonのログの3つを組み合わせて、原因を1つずつ切り分けるデバッグ方法を身につけました。それぞれの層の役割を意識しながらデータを追うことで、複雑なシステムでも冷静に問題の場所を見つけられるようになりました。

### 2. Dockerによる多層環境の構築（コンテナ間の通信）

* **課題**
フロント・バック・データベース・AI解析・nginxという5つの異なる環境（コンテナ）を1つのDockerネットワーク内で共存させ、お互いに正しく通信させる設定に苦労しました。
* **解決策**
Docker内では「コンテナ名そのものが通信相手の住所（ホスト名）になる」という仕組みを理解することで解決しました（例：`http://python:8000`）。これにより、お互いの環境が迷わずにデータをやり取りできるようになり、複雑なネットワーク設定をスッキリとまとめることができました。

### 3. TypeScriptによる厳密な型定義の徹底

* **課題**
API（サーバー）から返ってくる複雑な求人データやユーザー情報に対して、画面側（TypeScript）で厳密な「型（データのルール）」を定義することに挑戦しました。最初はルールが厳しすぎて型エラーが多発し、苦戦しました。
* **解決策**
データを受け取るためのルールをあらかじめ1つずつ細かく決めておくことで、流れてくるデータの中身が明確になりました。結果として、値のすれ違いや存在しないデータを参照しようとするバグを未然に防ぐことができ、型をしっかり決めることの大切さを体感しました。

### 4. LaravelのPolicy・Middlewareによる細かな権限（ロール）制御

* **課題**
「求職者が企業の求人を編集できないようにする」「企業が他社の求人を操作できないようにする」といった、ユーザーの立場（ロール）に応じた細かいアクセス制限の仕組みづくりに悩みました。
* **解決策**
Laravelのアクセス制限機能（Policy）や、通信を途中でチェックする仕組み（カスタムMiddleware）を組み合わせることで、裏側のAPIルートまで強固に守るセキュリティを実装しました。また、URLの文字とプログラム側の変数名を一致させるというLaravel独特の命名ルールにもハマりましたが、原因を突き止めたことで、フレームワークの自動化の仕組みへの理解がより深まりました。


---
## AI診断の仕組み
 
このアプリのAI分析は **3段階** で動いています。用途によって使い分けているのがポイントです。
...
### 段階3 : 最新トレンド分析（外部連携）— Python × Gemini 2.5 (Google Search)
特定のスキル名を入力すると、AIがインターネット上の最新動向を調査。「2026年の今、その技術はどう使われているか」「初心者が次に学ぶべき3つのこと」を、求職者の視点に立って優しく解説します。
...
| | LangChain | AIエージェントの拡張（今後の拡張基盤として導入） |
